...
from ..utils import PanicError, load_chars_file
...
class FontManager:
    def __init__(self,
        ...
        if self.font_list_file is not None:
            with open(str(self.font_list_file), 'r', encoding='utf-8') as f:
                lines = f.readlines()
    ...
    def _load_font_support_chars(self):
        ...
        for font_path in self.font_paths:
            ...
            try:
                ...
                logger.error(f"Load font file {font_path} failed, skip it. Error: {ee}")
